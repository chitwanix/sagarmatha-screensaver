...
sagarmatha-screensaver-command --exit

# run the daemon in the debugger
#gdb --args sagarmatha-screensaver --no-daemon --debug --sync

# or if that isn't helpful just get the debug output
#sagarmatha-screensaver --no-daemon --debug > /tmp/gs-debug-log.txt 2>&1

# or just run it with debugging on
sagarmatha-screensaver --no-daemon --debug
